...
export async function pin() {
    ...
    for (const paperDir of paperDirs) {
        ...
        for (const version of versions) {
            ...
            try {
                const output = execSync(`ipfs add --cid-version=1 --pin=true --recursive --quiet "${versionPath}"`, {
                    encoding: "utf-8"
                });
    ...
    if (paperCids.length > 0) {
        for (const { paperdir, cid } of paperCids) {
            ...
            try {
                execSync(`ipfs cat "${cid}" > /dev/null 2>&1 || ipfs ls "${cid}" > /dev/null 2>&1`, { stdio: 'pipe' });
                ipfsAccessible = true;
            ...
            if (ipfsAccessible) {
                ...
                try {
                    const output = execSync(`ipfs add --cid-version=1 --pin=true --recursive --quiet "${path.join(papersDir, paperdir)}"`, {
                        encoding: "utf-8"
                    });
